...
$CGI::CMS::Translate::VERSION = '0.37';
...
sub getSelf {
    return @_
        if defined( $_[0] )
            && ( !ref( $_[0] ) )
            && ( $_[0] eq 'CGI::CMS::Translate' );
    return (
        defined( $_[0] )
            && ( ref( $_[0] ) eq 'CGI::CMS::Translate'
            || UNIVERSAL::isa( $_[0], 'CGI::CMS::Translate' ) )
    ) ? @_ : ( $CGI::CMS::Translate::DefaultClass->new, @_ );
}
